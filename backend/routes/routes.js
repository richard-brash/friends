...
router.post('/', authenticateToken, authorizeRoles('admin', 'coordinator'), async (req, res) => {
	try {
		...
		const route = await routeService.createRoute({ name, description, color });
...
router.patch('/:id', authenticateToken, authorizeRoles('admin', 'coordinator'), async (req, res) => {
	try {
		const { id } = req.params;
		const { name, description, color } = req.body;
		const route = await routeService.updateRoute(Number(id), { name, description, color });
...
router.delete('/:id', authenticateToken, authorizeRoles('admin'), async (req, res) => {
	try {
		const route = await routeService.deleteRoute(Number(req.params.id));
		if (!route) return res.status(404).json({ error: 'Route not found' });
		res.sendStatus(204);
	} catch (error) {
